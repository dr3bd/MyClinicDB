...
export default function App() {
  ...
  return (
    ...
    >
      ...
      {activeSection === 'patients' && <DataTable data={patients} columns={patientColumns} locale="ar" />}
      {activeSection === 'appointments' && <DataTable data={appointments} columns={appointmentColumns} locale="ar" />}
      {activeSection === 'billing' && <DataTable data={invoices} columns={invoiceColumns} locale="ar" />}
      {!repositories && <EmptyState title="جاري التحميل" description="يتم تجهيز البيانات في وضع عدم الاتصال" />}
    </AppShell>
  );
}
